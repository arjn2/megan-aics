...
class StyleGANConfig:
    ...
    epochs: int = 15  # Set epochs to 15
...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        if epoch % 3 == 0:
            ...
            train_accuracy = evaluate_model(detection_model, train_ds, class_indices)
            val_accuracy = evaluate_model(detection_model, val_ds, class_indices)
            print(f"Epoch {epoch+1} Training Accuracy: {train_accuracy:.4f}")
            print(f"Epoch {epoch+1} Validation Accuracy: {val_accuracy:.4f}")
            print(f"Epoch {epoch+1} Variant Loss: {avg_losses['variant_loss']:.4f}")
            print(f"Epoch {epoch+1} Generator Loss: {avg_losses['generator_loss']:.4f}")
