...
def evaluate_gan(model, val_ds):
    correct_predictions = 0
    total_predictions = 0

    for images, labels in val_ds:
        z = tf.random.normal((images.shape[0], config.latent_dim))
        generated_images = model.generator(model.mapping_network([z, labels]))
        predictions = model.discriminator([generated_images, labels])
        correct_predictions += tf.reduce_sum(tf.cast(tf.argmax(predictions, axis=1) == tf.argmax(labels, axis=1), tf.float32))
        total_predictions += images.shape[0]

    accuracy = correct_predictions / total_predictions
    print(f"GAN Accuracy: {accuracy:.4f}")
    return accuracy
...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        # Evaluate GAN accuracy
        gan_accuracy = evaluate_gan(model, val_ds)
