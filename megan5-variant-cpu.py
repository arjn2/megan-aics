...
class MalwareStyleGAN:
    ...
    def train_step(self, real_images, labels):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # Use TensorArray for variants
            variants = tf.TensorArray(tf.float32, size=self.config.num_variants)
            w_variants = tf.TensorArray(tf.float32, size=self.config.num_variants)
            
            # Store base variant
            variants = variants.write(0, self.generator(w_base))
            w_variants = w_variants.write(0, w_base)
            
            # Generate additional variants
            for i in tf.range(1, self.config.num_variants):
                z_new = tf.random.normal((batch_size, self.config.latent_dim))
                w_new = self.mapping_network([z_new, labels])
                w_mixed = w_base * 0.5 + w_new * 0.5  # Fixed mixing ratio for stability
                variants = variants.write(i, self.generator(w_mixed))
                w_variants = w_variants.write(i, w_mixed)
            
            variants_stacked = variants.stack()
            ...
            # Loss calculations
            variant_labels = tf.eye(self.config.num_variants)
            variant_labels = tf.tile(variant_labels, [batch_size, 1])
            
            variant_loss = tf.reduce_mean(
                tf.keras.losses.categorical_crossentropy(
                    variant_labels, variant_logits, from_logits=True))
            
            diversity_loss = -tf.reduce_mean(tf.abs(
                variants_stacked[:, None] - variants_stacked[None, :]))
...
def main():
    ...
    # Get class indices from the raw dataset
    ...
    for epoch in range(config.epochs):
        ...
        # Log epoch metrics
        avg_losses = {k: float(np.mean([x[k] for x in epoch_losses])) for k in epoch_losses[0].keys()}
